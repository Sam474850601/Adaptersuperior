package com.to8to.app.adaptersuperior.compiler;

/**
 * Created by same.li on 2018/4/3.
 */

public class ClassNames {

    public static final String RECYCLERVIEW  ="android.support.v7.widget.RecyclerView";

    public static final String VIEWHOLDER  ="ViewHolder";


    public static final String PACKAGE_VIEW  ="android.view";

    public static final String VIEW  ="android.view.View";

    public  static  final String PACKAGE_NAME = "com.to8to.app.adaptersuperior";

    public static final String ADAPTER_SUPPORTDEFAULT = "com.to8to.app.adaptersuperior.lib.SupportDefaultAdapter";


    public static final String APP_UTIL = PACKAGE_NAME+".lib.AdapterSuperiorAppUtil";


    public static final String APP_UTIL__GET_IDENTIFIER = "getIdentifier";

    public static final String APP_UTIL__GET_LAYOUTRESID = "getLayoutResId";

    public static final String APP_UTIL__GET_VIEWRESID = "getViewResId";






    public static final String EMPTYADAPTERHOLDER = PACKAGE_NAME+".lib.EmptyAdapterHolder";

    public static final String INJECT_HANDLER =  "AdapterSuperiorHandler";

    public static final String INJECT_HANDLER_INIT =  "init";




}
